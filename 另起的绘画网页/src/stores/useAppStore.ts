...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  reclusterFrameFromScratch: (frameId) => {
    ...
    if (frame.bbox && frame.bgImageData) {
      const baseColorList = [...palette.entries()].map(([id, c]) => ({ id, h: c.h, s: c.s, l: c.l }));
      const newDeltaPacked = frame.deltaPacked
        ? new Uint16Array(frame.deltaPacked)
        : new Uint16Array(currentTex.length);
      const tinyResult = mergeTinyRegions(
        ...
        frame.bbox,
        frame.bgImageData,
        frame.bgImageData.width,
        frame.blockFlags ?? 0n,
        10,
        0.02,
      );
      if (tinyResult.mergedPixels > 0) {
        // 维护调色板引用计数（被归并清除的小色 id）
        for (const oldId of tinyResult.removedIds) {
          get().decrementColorRef(oldId, frameId);
        }
        ...
        console.log('[重新聚类] 小量颜色归并：归并 ' + tinyResult.mergedPixels + 'px，校验不通过保留 ' + tinyResult.keptPixels + 'px，清除小色 ' + tinyResult.removedIds.length + ' 个');
      }
    }
