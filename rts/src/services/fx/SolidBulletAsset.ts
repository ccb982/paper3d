...
/** 法球 HSL 四段（由内到外） */
const BOLT_CORE: readonly [number, number, number] = [0.13, 1.00, 0.86]; // 白热黄
const BOLT_MID: readonly [number, number, number] = [0.07, 1.00, 0.60];  // 橙焰
const BOLT_OUT: readonly [number, number, number] = [0.02, 0.95, 0.42]; // 暗红橙
const BOLT_RIM: readonly [number, number, number] = [0.00, 0.90, 0.26]; // 焦红
...
export function createFireballAsset(size = 64): FrameAssetSource {
  const n = size * size;
  const baseData = new Float32Array(n * 4);
  const resData = new Uint8Array(n * 4);
  const c = size / 2;
...
/** ★ 法球世界直径（米）。正方形纹理 ⇒ 宽高同值 */
export const FIREBALL_BASE_WIDTH = 0.8;
