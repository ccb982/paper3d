...
//   权威架构文档：《地形与渲染管线架构.md》（定稿 2026-09-05）
// ============================================================
...
//
// ★ 消费方（《RTS架构.md》§6.0 表管线）：
//   finalRuling → 渲染/几何/物理（坡面/立面）+ **可行性表 PassTable**（敌人导航收敛；
//   再往下才是寻路 LocalStep/Route 与移动 CharacterCore）。本文件 = **真相源**；
//   消费端（PassTable/语义表/寻路/移动）只读裁决，不得改判。
...
/**
 * ★ 通高硬边界阈值（|Δh| ≤ 带宽 → 小台阶语义）。推导见《地形与渲染管线架构.md》
 * 第一部分（原边缘裁决档 §2.2）：各类型对 |Δh| 分布中 (0.30, 0.38) 是空隙带。
...
//   架构：《地形与渲染管线架构.md》§4（2026-08-31 后修正+角边融合）
