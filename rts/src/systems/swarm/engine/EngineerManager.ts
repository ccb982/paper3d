...
// 工兵**由新引擎全权负责**（旧工事链已销毁）。
//
// ★★ 工兵流程（用户设计；唯一口径 2026-09-25）★★
//   ① 取件：引擎给一个**可行的**建造位置（可行 = 有需求 + 在施工带内 + 队可达）；
//   ② 施工：队只管过去造（队长到件 3m 内计时；掩体 6s / 战壕 10s，满即落地）；
//   ③ 续件：造完立刻申请下一个可行位置，接着造；
//   ④ 只要引擎不干预（无事态变化/无玩家令/无收回），就一直造下去。
//   本文件负责 ①③（取件/续件）；② 的落地执行由队长层（SquadCore）+ 本类计时共同完成。
//   现状偏差（待查）：队常在前沿取不到件 → 站桩 → 被收回（见 §0.2 汇报）。
//
...
//
// ★★ 收回机制铁律（《RTS架构.md》§0.1）★★
//   工兵被卡死收回 = **一定出了问题**（站桩/无件可派/到不了件/施工不推进）——
//   修派件与行动，**绝不给工兵开豁免**；宁可错杀不可放过。
//
// ★★ 禁私自补丁铁律（《RTS架构.md》§0.2）★★
//   只做架构级调整；发现问题先汇报（现象/证据/根因/影响面/方案），不得私自加兜底/特判；
//   未经裁决不得改动本文件行为。
// ============================================================
