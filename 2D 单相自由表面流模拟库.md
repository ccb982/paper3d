...
| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `maxLifetime` | `number` | 流体最大寿命（秒），0表示禁用 | 3.0 |
...
爆炸系统用于在流体中产生冲击效果，可以生成水花四溅或仅吹动已有水体。
...
| `strength` | `number` | 爆炸强度 | - |
| `createWater` | `boolean` | 是否生成新水（true=生成水花，false=仅加速已有水） | `true` |
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  1. 计算水量因子（基于 Level Set 的 phi 值）                               │
│     waterMass = max(0.01, -phi)  // 深水区质量大，浅水区质量小            │
│     impulse = strength * mask                                                │
│                                                                             │
│  2. 方向计算                                                                │
│     radialDir = normalize(uv - center)                                       │
│     perp = (-radialDir.y, radialDir.x) * hash                               │
│                                                                             │
│  3. 速度叠加（动量守恒：p = mv）                                            │
...
│     → 质量越小，速度越大（浅水迸溅）                                         │
│                                                                             │
│  4. 水位变化（根据水深调整，不易吹散厚水区）                                │
│     generationFactor = clamp(1.0 - waterMass * 2.0, 0.0, 1.0)               │
│     → 薄水区更容易被补充成水花                                               │
│     → 深水区不容易被加深                                                     │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
| 水层状态 | phi 值 | waterMass | 获得的速度 | 视觉效果 |
|----------|--------|-----------|------------|----------|
| 深水区 | -0.5 | 0.5 | 低 | 微微震荡，保持厚重感 |
| 中水区 | -0.2 | 0.2 | 中等 | 正常流动 |
| 浅水/边缘 | -0.05 | 0.05 | 高 | 高速迸溅，产生水花 |

#### 使用示例

```typescript
// 生成水花四溅（在指定位置创造新水并产生冲击波）
// 小半径 + 高强度 = 突破压力投影阻尼，产生明显飞溅效果
simulator.explode(0.5, 0.5, 0.08, 500.0, true);

// 只吹动已有的水（不产生新水，适合风场或扰动效果）
simulator.explode(0.5, 0.3, 0.12, 25.0, false);
```
...
|------|----------|
| **径向扩散** | 速度方向指向爆炸中心的反方向 |
| **随机扰动** | 使用 hash 函数添加垂直于径向的随机分量 |
| **平滑过渡** | 使用 smoothstep 实现边缘平滑衰减 |
| **双重模式** | createWater 参数控制是否生成新水 |
| **无速度限制** | 已移除全局速度限制，允许爆炸产生高速度 |

#### 爆炸力优化说明

**压力投影的消能效应**：爆炸会在小区域内瞬间制造高速度梯度，产生巨大散度（div），随后压力求解会生成强压力场，在速度修正步骤中抵消大部分径向速度。

**实现飞溅效果的关键参数**：

| 参数配置 | 效果 |
|----------|------|
| 小半径（0.08-0.12） | 减少散度累积，降低压力阻尼效果 |
| 高强度（50.0-500.0） | 让速度在压力反应前把水推出去 |
| createWater=true | 生成新水并施加力，产生水花 |

**注意**：已移除 `externalForcesMat` 中的全局速度限制（原 maxVel=30.0），确保爆炸产生的高速度可以完全传递给流体。
