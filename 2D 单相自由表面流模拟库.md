...
| `duration` | 爆炸持续时间（秒），默认 0.1。使用高斯脉冲包络，强度先升后降 |
...
| **高斯脉冲包络** | 爆炸强度遵循 exp(-t²/(2σ²))，先升后降的自然衰减 |
...
3. **寿命系统交互（TODO）**:
   - 当前代码在生成水花时未重置年龄
   - 建议增加额外 Pass 将新产生水区域（phi 刚转为负值）的年龄重置为 0
