...
| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `maxLifetime` | `number` | 流体最大寿命（秒），0表示禁用 | **3.5** |
...
| `strength` | `number` | 散度源强度（推荐5000~10000） | - |
...
| 参数 | 详细说明 |
|------|----------|
| `strength` | 控制散度源强度，**不再对应直接的速度增量**，推荐值 5000~10000 |
...
export interface FluidParams {
    ...
    usePCG?: boolean;           // ⚠️ PCG求解器已创建但未启用，当前使用Jacobi
    
    // 寿命系统参数
    maxLifetime?: number;       // 流体最大寿命（秒），0表示禁用寿命系统，默认3.0
}
...
export class FluidSimulator {
    ...
    // PCG求解器纹理（⚠️ 已创建但未启用，当前使用Jacobi迭代）
