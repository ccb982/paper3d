...
│     记录: cx, cy, radius, strength, createWater, startTime, duration       │
│                                                                             │
...
│         S = strength * envelope * mask(x)  // 简化散度源公式               │
...
1. **散度源公式**: `S = strength * envelope(t) * mask(x)`
   - `envelope(t)`: 二次抛物线 `4*u*(1-u)`，u=t/duration，峰值在中间(0.5)，先升后降
   - `mask(x)`: 径向衰减掩码，`1 - smoothstep(0, radius, dist)`
   - **简化公式**: 移除了原公式中的 `/ (ρ * Δt)` 因子，避免数值爆炸问题
   - **强度推荐值**: 5000~20000，根据视觉效果调整
...
4. **材质预缓存优化**:
   - `explosionDivMat`、`waterGenMat`、`ageResetMat` 预创建并复用
...
6. **Jacobi边界处理**:
   - 在压力Jacobi迭代中，若邻居为空气(phi>0)或固体(isSolid>0.5)，用中心压力代替
   - 相当于 Neumann 边界条件 `∂p/∂n = 0`，确保自由表面附近压力场准确
