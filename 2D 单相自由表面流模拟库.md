...
│         S = strength * envelope * mask(x) / (ρ * Δt)  // 散度源项         │
...
1. **散度源公式**: `S = k * envelope(t) * strength / (ρ * Δt)`
   - `envelope(t)`: 二次抛物线 `4*u*(1-u)`，u=t/duration，峰值在中间(0.5)，先升后降
   - `k`: 缩放因子，mask(x) 的积分归一化
   - **数学一致性**: S 已除以 `(ρ*Δt)`，加入散度后乘以压力Poisson右端项的 `(ρ/Δt)`，最终得到 `k*envelope*strength`
...
3. **水花速度赋值**:
   - 新生成水花的速度应设为周围流体速度的加权平均，而非保持为0
   - 建议在生成水花的同一 Pass 中完成速度赋值，避免水花"粘腻"
...
5. **寿命系统交互（TODO）**:
   - 当前代码在生成水花时未重置年龄，若在已有水中爆炸，新生水花可能继承旧年龄
   - 建议增加额外 Pass：检测 phi 刚转为负值的像素，将其年龄重置为 0
