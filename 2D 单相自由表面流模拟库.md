...
export class FluidSimulator {
    ...
    public update(deltaTime?: number): void {
        ...
        // 8.5. 流体寿命系统（当 maxLifetime > 0 时启用）
        if (this.params.maxLifetime && this.params.maxLifetime > 0) {
            ...
            // 步骤1: 年龄平流 - 让年龄跟随水流移动（关键：追踪水滴而非像素）
            const ageAdvectMat = this.ageAdvectionShader();
            ageAdvectMat.uniforms.dt.value = this.params.timeStep;  // 物理步长用于平流
            this.renderFullscreen(ageAdvectMat, { 
                age: this.curAgeTex.texture, 
                velocity: this.curVelTex.texture, 
                levelset: this.curPhiTex.texture 
            }, this.curAgeTex === this.ageTexA ? this.ageTexB : this.ageTexA);
            this.curAgeTex = this.curAgeTex === this.ageTexA ? this.ageTexB : this.ageTexA;
            ageAdvectMat.dispose();

            // 步骤2: 年龄更新 - 增加年龄（使用真实时间）+ 注入区域重置
            const ageUpdateMat = this.ageUpdateShader();
            ageUpdateMat.uniforms.dt.value = realDelta;  // 使用真实时间增量
            this.renderFullscreen(ageUpdateMat, { 
                age: this.curAgeTex.texture, 
                levelset: this.curPhiTex.texture 
            }, this.curAgeTex === this.ageTexA ? this.ageTexB : this.ageTexA);
            this.curAgeTex = this.curAgeTex === this.ageTexA ? this.ageTexB : this.ageTexA;
            ageUpdateMat.dispose();

            // 步骤3: 流体消散（基于年龄）
            const dissipationMat = this.dissipationShader();
            this.renderFullscreen(dissipationMat, { 
                levelset: this.curPhiTex.texture, 
                age: this.curAgeTex.texture 
            }, this.phiTexA);
            this.curPhiTex = this.phiTexA;
            dissipationMat.dispose();
        }
