...
轻量级流体实体（`LightFluidEntity`）允许将流体模拟作为独立实体被 `EntityManager` 管理，适用于模拟水滴、碎片等小型流体效果。

#### 核心设计要点

| 特性 | 实现方式 |
|------|----------|
| **独立模拟器** | 内部包含一个小型 FluidSimulator（32x32） |
| **实体集成** | 继承自 Entity 基类，可被 EntityManager 管理 |
| **物理集成** | 支持世界空间速度、力、冲量 |
| **生命周期** | 自动超时销毁（默认5秒） |
| **资源管理** | 自动释放内部模拟器和材质资源 |
| **LOD支持** | 支持距离级别细节控制 |
...
export class LightFluidEntity extends Entity {
    ...
    public worldVelocity: THREE.Vector3; // 世界空间速度
    public maxAge: number = 5;           // 最大生命周期（秒），默认5秒
    public lod: FluidLOD = FluidLOD.HIGH; // LOD等级
    ...
    // 核心方法
    update(delta: number): void;                // 每帧更新（含LOD控制）
    applyForce(force: THREE.Vector3, delta: number): void;  // 施加力
    applyImpulse(impulse: THREE.Vector3): void; // 施加冲量
    setVelocity(velocity: THREE.Vector3): void; // 设置速度
    isEmpty(): boolean;                         // 检查是否消散
    getSimulator(): FluidSimulator;             // 获取内部模拟器
}
```
...
```typescript
import { LightFluidEntity, FluidLOD } from '@entities/fluid';
...
const droplet = new LightFluidEntity(
    'droplet_1', 
    renderer, 
    new THREE.Vector3(0, 2, 0),  // 初始位置
    new THREE.Vector3(0, -2, 0), // 初始速度
    0.15                         // 缩放大小
);

// 设置LOD级别
droplet.lod = FluidLOD.MEDIUM;
...
EntityManager 新增液滴管理方法：

| 方法 | 说明 |
|------|------|
| `setRenderer(renderer)` | 设置渲染器引用 |
| `createDroplets(count, centerPos, spreadRadius, velocity, scale)` | 批量创建液滴（异步） |
| `createDroplet(id, position, velocity, scale)` | 创建单个液滴（异步） |
| `getDroplets()` | 获取所有液滴实体 |
| `updateWithPlayer(delta, playerPosition)` | 带玩家位置的更新（用于LOD计算） |

---

### 新增功能：流体区域管理器 ⭐

`FluidRegionManager` 用于管理一片区域内的多个小液滴实体，支持基于距离的 LOD 性能优化和安全的实体生命周期管理。

#### 核心设计要点

| 特性 | 实现方式 |
|------|----------|
| **区域管理** | 管理一组相关的 LightFluidEntity |
| **LOD控制** | 根据距离动态调整模拟精度 |
| **安全删除** | 标记-延迟删除机制，避免迭代时修改数组 |
| **生命周期** | 距离过远时自动销毁区域 |
| **事件通知** | 状态变更回调 |
...
由于 EntityManager 和 FluidRegionManager 互相依赖，采用动态导入打破循环：
...
为避免在迭代期间修改集合导致的运行时错误，采用"标记-延迟删除"模式：

1. **标记阶段**：遍历实体时，将需要删除的实体标记为 `isActive = false`
2. **收集阶段**：收集所有标记为待删除的实体/区域
3. **删除阶段**：在循环外部统一删除收集的实体

```typescript
// 在 EntityManager.updateEntities 中
...
当创建多个 FluidSimulator 实例时可能出现 WebGL 反馈循环错误，解决方案是确保只初始化一次流体区域：
...
渲染层着色器中添加了速度阈值控制，当流体速度超过设定阈值时，纹理会变白显示高速流动效果。

#### 参数配置

| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `whiteSpeedThreshold` | `number` | 速度阈值，超过此值纹理变白 | **50.0** |

#### 实现位置

在 `FluidSimulator.ts` 的渲染着色器中，速度可视化逻辑如下：

```typescript
// 在 getRenderMaterial() 中更新
uniform float whiteSpeedThreshold;  // 默认 50.0

// 着色器中速度检测
float speed = length(vel);
if (speed > whiteSpeedThreshold) {
    // 高速区域变白
    color.rgb = mix(color.rgb, vec3(1.0), smoothstep(whiteSpeedThreshold, whiteSpeedThreshold * 1.5, speed));
}
```
...
为解决游戏内计时与实际时间不同步的问题，在 `EntityManager` 中添加了全局时间缩放因子。
...
| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `timeScale` | `number` | 时间缩放因子，1.0=正常速度 | **2.0** |

#### 使用场景

- **加快游戏速度**：当游戏帧率过低导致计时变慢时，可通过增大 `timeScale` 补偿
- **调试模式**：可设置为更高值快速测试生命周期逻辑
- **慢动作效果**：可设置为小于1的值实现慢动作

---

### 修复功能：流体寿命系统修复 ⭐

修复了 `FluidSimulator` 中 `maxLifetime` 参数的空值判断问题。
...
```typescript
// 错误：当 maxLifetime = 0（表示不消散）时，被当作 falsy 值
maxLifetime: { value: this.params.maxLifetime ?? 10.0 },
```

当 `maxLifetime` 设置为 `0`（表示"不消散"）时，会被错误地解释为未设置，使用了默认值 `10.0`，导致流体总是在10秒后消散。

#### 修复方案

```typescript
// 正确：只检查 undefined，0 是有效的设置值
...
| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `maxLifetime` | `number` | 流体最大寿命（秒），0表示不消散 | **4.0** |

---

### 优化功能：轻量级流体实体性能优化 ⭐

针对 `LightFluidEntity` 进行了多项性能优化，降低 CPU 和 GPU 开销。
...
| LOD等级 | 更新频率 | 说明 |
|---------|----------|------|
| HIGH | 30 FPS (1/30) | 近处液滴，较高频率 |
| MEDIUM | 20 FPS (1/20) | 中等距离 |
| LOW | 12 FPS (1/12) | 远处液滴，较低频率 |
| OFF | 停止更新 | 超过10单位，标记销毁 |
...
const params: FluidParams = {
    pressureIterations: 1,        // 减少压力迭代次数（原为3）
    reinitIterations: 0,          // 关闭重初始化（原为1），小水滴表面张力不重要
    timeStep: 0.01,               // 增大时间步长（原为0.005）
    usePCG: true,                 // 使用PCG求解器加速压力计算
    maxLifetime: 0,               // 不使用内部消散，由实体生命周期控制
};
```

#### 实例数量限制

`FluidRegionManager` 限制同时活跃的模拟实例数：

```typescript
private maxActiveSimulations: number = 4;  // 最多同时运行4个模拟实例
```

超出限制的液滴会被强制设置为 `FluidLOD.OFF`，暂停模拟。
...
export interface FluidParams {
    ...
    // 流体居中参数
    enableCentering?: boolean;       // 是否启用流体居中（默认false）
    centeringInterval?: number;      // 居中计算间隔（帧），默认60
    centeringThreshold?: number;     // 居中阈值（UV空间），默认0.05
}
...
水面纹理分裂系统允许将主水面分裂成多个独立的小液滴碎片，增强爆炸和破坏效果。

| 特性 | 实现方式 |
|------|----------|
| **GPU加速分裂** | 使用着色器生成裂缝掩码，高效分离扇形区域 |
| **独立实体** | 分裂后的碎片成为独立的 `LightFluidEntity` |
| **自动生命周期** | 碎片有独立的寿命，自动销毁释放资源 |
| **物理连续性** | 保留原始速度场和Level Set数据 |
...
export class FluidFragmentSystem {
     ...
     * @param cx 中心X（UV坐标）
     * @param cy 中心Y（UV坐标）
     * @param radius 分裂半径（UV空间）
     * @param strength 爆炸强度
     * @param crackCount 裂缝数量（默认4）
     * @param seed 随机种子（可选，用于确定性重放）
     * @param worldCenter 世界坐标中心（可选）
     * @returns 创建的碎片实体列表
     */
    explode(
        cx: number, cy: number, radius: number, strength: number,
...
```typescript
// 在 FluidSimulatorAdapter 中初始化和使用
const fragmentSystem = new FluidFragmentSystem(
    simulator,
    entityManager,
    (uv) => new THREE.Vector3(uv.x * 10 - 5, uv.y * 10 - 5, 0)
);

// 分裂水面（中心位置，分裂成6个碎片）
const fragments = fragmentSystem.explode(0.5, 0.5, 0.2, 8.0, 6);
console.log(`分裂完成，创建了 ${fragments.length} 个碎片`);
```
...
│  阶段1: 生成裂缝掩码（GPU）                                                  │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  generateCrackMaskGPU(targetRT, centerUV, angles, angleWidth, seed)│   │
│  │  → 在指定角度位置生成裂缝图案                                       │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                              │                                             │
│                              ▼                                             │
...
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  for each sector:                                                  │   │
│  │      createFragmentDroplet(phiData, velData, worldPos, worldVel)   │   │
│  │      → 创建 LightFluidEntity（32x32 小型模拟器）                   │   │
│  │      → 添加到 EntityManager                                        │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                              │                                             │
│                              ▼                                             │
│  阶段5: 清理主水面区域                                                      │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  clearSectorRegionsGPU(centerUV, sectors, radius)                  │   │
│  │  → 清除主水面上被分裂出去的区域                                     │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
流体居中系统自动检测流体质心，并将其平移到纹理中心，防止流体漂移出边界。

| 特性 | 实现方式 |
|------|----------|
| **GPU质心计算** | 使用归约着色器高效计算流体中心位置 |
| **自适应频率** | 按固定间隔计算，避免每帧开销 |
| **平滑平移** | 使用双缓冲实现纹理平滑平移 |
| **阈值控制** | 只有当偏移超过阈值时才执行居中 |

#### 关键方法

```typescript
public updateCentering(): void;  // 更新居中（每N帧调用一次）
public getCentroid(): THREE.Vector2 | null;  // 获取当前质心
public enableCentering(enable: boolean): void;  // 启用/禁用居中
```
...
1. **循环依赖**：`EntityManager` 和 `FluidRegionManager` 互相依赖，导致编译错误
2. **代码重复**：液滴生成逻辑同时存在于两个类中

#### 解决方案

**1. 类型抽取**：创建独立的类型文件 `FluidTypes.ts`

```typescript
// src/entities/fluid/FluidTypes.ts
export enum FluidLOD {
    HIGH = 'high',
    MEDIUM = 'medium',
    LOW = 'low',
...
**2. 统一液滴生成**：将液滴创建逻辑统一到 `FluidRegionManager`

```typescript
// FluidRegionManager.createDroplets() 作为唯一入口
// EntityManager 调用 FluidRegionManager 创建液滴
```
...
| **时间保护** | 在 `update(dt)` 时统一处理力，速度注入乘以 dt 转换为加速度模型 |
| **与重力一致** | 外力遵循与重力相同的模型：`vel += acceleration * dt` |
| **散度保持不变** | 散度注入本身是脉冲量，直接施加不缩放 |
| **双通道支持** | 同时支持 `FluidSimulatorAdapter` 和 `LightFluidEntity` |
...
export interface FluidExternalForce {
    ...
    /** 速度注入（解释为加速度，内部乘以 dt） */
    velocityInjection?: {
        velocity: THREE.Vector2;      // 加速度 (uv/s²)
        radius?: number;              // 影响半径（UV空间），默认0.2
        centerUV?: THREE.Vector2;     // 中心位置（UV空间），默认(0.5, 0.5)
    };
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  【外部调用阶段】applyFluidForce(force)                                     │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  // 只缓存，不立即注入                                               │   │
│  │  if (force.velocityInjection || force.divergenceInjection ||        │   │
│  │      force.waterInjection) {                                        │   │
...
│  │          dvx = velocity.x * dt;                                   │   │
│  │          dvy = velocity.y * dt;                                   │   │
│  │          sim.addLocalVelocityImpulse(dvx, dvy, ...);               │   │
│  │      }                                                            │   │
│  │      if (force.divergenceInjection) {                              │   │
│  │          // 散度注入保持不变（脉冲量）                               │   │
│  │          sim.addDivergenceImpulse(...);                            │   │
│  │      }                                                            │   │
│  │  }                                                                │   │
│  │  // 清空队列                                                        │   │
...
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ✅ 核心优势: 无论外部调用频率如何，力的施加都与时间步 dt 绑定，           │
│              避免速度无限累积导致的数值不稳定                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
adapter.update(delta);  // 在 update 中统一处理
```
...
| 参数 | 类型 | 说明 | 推荐值 |
|------|------|------|--------|
| `velocity.x/y` | `number` | 加速度（uv/s²），内部会乘以 dt | 50~500 |
| `radius` | `number` | 影响半径（UV空间，0~1） | 0.2~1.0 |
| `centerUV` | `THREE.Vector2` | 力的中心位置（UV空间） | (0.5, 0.5) |

#### 与重力模型的对比

| 力类型 | 模型 | 每帧增量 |
|--------|------|----------|
| 重力 | `vel.y -= gravity * dt` | `9.81 * 0.002 ≈ 0.02 uv/s` |
| 外力 | `vel += acceleration * dt` | `200 * 0.002 = 0.4 uv/s` |

> **设计原则**：为避免侧向力"淹没"重力效果，建议将侧向加速度设置为重力的 2~10 倍（即 20~100 uv/s²）。
...
| 方法 | 说明 |
|------|------|
| `addVelocityImpulse(dvx, dvy)` | 全局速度注入（所有像素叠加速度） |
| `addLocalVelocityImpulse(dvx, dvy, radius, cx, cy, maxSpeed)` | 局部速度注入（带速度上限） |

**addLocalVelocityImpulse 参数说明**：

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `dvx` | number | - | X方向速度增量（UV空间） |
| `dvy` | number | - | Y方向速度增量（UV空间） |
| `radius` | number | 0.2 | 影响半径（UV空间，0~1） |
| `cx` | number | 0.5 | 中心X坐标（UV空间） |
| `cy` | number | 0.5 | 中心Y坐标（UV空间） |
| `maxSpeed` | number | 2.0 | 最大速度上限 |

**实现原理**：
- 使用 `mask = 1.0 - smoothstep(0.0, radius, d)` 创建渐变遮罩
- 只在指定中心和半径范围内叠加速度，边缘平滑过渡
- 叠加后计算速度大小，若超过 `maxSpeed` 则归一化到上限
...
**LOD 系统**：远离玩家的流体区域冻结纹理刷新，节省 GPU 资源

```typescript
// 高 LOD：每帧更新纹理
fluidSimulator.setTextureUpdateEnabled(true);

// 低 LOD：冻结纹理，画面静止
fluidSimulator.setTextureUpdateEnabled(false);
```
...
| 方法 | 说明 |
|------|------|
| `constructor(center, radius, maxDroplets)` | 创建区域管理器 |
| `addDroplet(droplet)` | 添加现有液滴 |
| `createDroplets(count, spread)` | 批量创建液滴 |
| `update(delta, lod)` | 更新区域（由 EntityManager 调用） |
| `getBounds()` | 获取区域边界 |
| `destroy()` | 销毁整个区域 |
| `getDropletCount()` | 获取液滴数量 |
| `getCurrentLOD()` | 获取当前 LOD 等级 |
...
// 创建区域管理器
const regionA = new FluidRegionManager(
    new THREE.Vector3(0, 0, 0),  // 中心位置
    3.0,                          // 半径
    15                            // 最大液滴数
);

// 注册到 EntityManager
EntityManager.getInstance().addFluidRegion(regionA);

// 创建液滴
await regionA.createDroplets(10, 2.5);

// 游戏循环中使用新的更新方法
entityManager.updateWithPlayer(delta, playerPosition);
```
...
纹理居中追踪需要计算水流体的中心位置，传统的包围盒计算方法需要回读整个 phi 纹理（32×32 = 4096 float），造成严重的 GPU-CPU 同步阻塞。

#### 解决方案

使用 GPU 归约计算水体质心，只回读 1×1 纹理（4 float），开销极小。

#### 核心算法

```typescript
private computeFluidCentroidGPU(): THREE.Vector2 | null {
    // 阶段1: 计算逐像素的加权值 phi*x, phi*y, phi
    const weightedMat = new THREE.ShaderMaterial({
        fragmentShader: `
            if (phi >= 0.0) {
                gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0);
            } else {
                float phiAbs = abs(phi);
    ...
    // 阶段2: 逐级归约（复用已有的 reduceTexPool）
    while (currentWidth > 1 || currentHeight > 1) {
        // ... 归约到 1x1
    }

    // 阶段3: 读取最终结果（只回读 4 float！）
    const pixelBuffer = new Float32Array(4);
    this.renderer.readRenderTargetPixels(reduceResult, 0, 0, 1, 1, pixelBuffer);
    ...
    return new THREE.Vector2(sumPhiX / sumPhi, sumPhiY / sumPhi);
}
```

#### 性能对比

| 方法 | 回读数据量 | GPU-CPU 同步开销 |
|------|-----------|------------------|
| 包围盒计算 (旧) | 32×32×4 = 4096 float | 高 |
| 质心归约 (新) | 1×1×4 = 4 float | **极低** |

#### 质心计算 vs 包围盒计算

| 特性 | 质心计算 | 包围盒计算 |
|------|----------|------------|
| **原理** | 加权平均 (Σφᵢxᵢ/Σφᵢ) | 边界框中心 |
| **抗干扰性** | 对噪声不敏感 | 对孤立噪点敏感 |
| **计算量** | 稍多（需加权） | 较少（只需边界检测） |
| **适用场景** | 分散/不规则水体 | 紧凑/球形水体 |