...
exports.ROLE_PIT = exports.ROLE_LIQUID = exports.ROLE_WALL = exports.ROLE_PATH = exports.BLOCKS_PER_SIDE = exports.BLOCK_SIZE = exports.CHUNK_SIZE = exports.hash2 = void 0;
exports.registerSpecialLayout = registerSpecialLayout;
exports.generateChunk = generateChunk;
const Tiles_1 = require("./Tiles");
const Tiles_2 = require("./Tiles");
const TileGroups_1 = require("./TileGroups");
...
exports.ROLE_PATH = 0; // 地面位（迷宫走廊/端口/死路平地）
exports.ROLE_WALL = 1; // 高台位（迷宫墙）
exports.ROLE_LIQUID = 2; // 液体位
exports.ROLE_PIT = 3; // 坑洞位
const SPECIAL_LAYOUTS = [];
/** ★ 扩展点：注册特殊 chunk 布局（稀有度由 match 内部盐控制） */
function registerSpecialLayout(entry) {
    SPECIAL_LAYOUTS.push(entry);
}
function resolveSpecialLayout(seed, cx, cz) {
    for (const e of SPECIAL_LAYOUTS) {
        if (e.match(seed, cx, cz))
            return e.build(seed, cx, cz);
    }
    return null;
}
// ============ 阶段 0：端口派生 ============
...
function structureSlots(seed, cx, cz, passage, ports) {
    const N = 225;
    const roles = new Uint8Array(N); // 默认 ROLE_PATH(0) —— 与旧 UNKNOWN→ROAD 收敛一致
    const portSet = new Set([...ports.top, ...ports.bottom, ...ports.left, ...ports.right]);
    // 1. 标记所有 passage 为 PATH（迷宫走廊）
    for (let i = 0; i < N; i++) {
        if (passage[i])
            roles[i] = exports.ROLE_PATH;
    }
    // 2. 端口强制 PATH
    for (const p of portSet) {
        roles[p] = exports.ROLE_PATH;
    }
    // 3. 收集墙区（既非走廊也非端口）→ 这些将成为高台位，少量被液体/坑洞位替换
    const wallSet = new Set();
    for (let i = 0; i < N; i++) {
        if (!passage[i] && !portSet.has(i))
            wallSet.add(i);
    }
    if (wallSet.size === 0)
        return roles;
...
function repairConnectivityRoles(roles, ports) {
    const allPorts = [...new Set([...ports.top, ...ports.bottom, ...ports.left, ...ports.right])];
    for (const p of allPorts) {
        if (roles[p] === exports.ROLE_LIQUID || roles[p] === exports.ROLE_PIT) {
            roles[p] = exports.ROLE_PATH;
        }
    }
}
...
function buildPortNearSet(ports) {
    const s = new Set();
    const allPorts = [...new Set([...ports.top, ...ports.bottom, ...ports.left, ...ports.right])];
    for (const p of allPorts) {
        s.add(p);
...
function fillSlots(seed, cx, cz, roles, ports, blockIds) {
    ...
    for (let i = 0; i < 225; i++) {
        switch (roles[i]) {
            case exports.ROLE_WALL:
                blockIds[i] = (0, TileGroups_1.drawTileForRole)(panel, 'platform', seed, cx, cz, i).id;
                break;
            case exports.ROLE_LIQUID:
                blockIds[i] = (0, TileGroups_1.drawTileForRole)(panel, 'liquid', seed, cx, cz, i).id;
                break;
            case exports.ROLE_PIT:
                blockIds[i] = (0, TileGroups_1.drawTileForRole)(panel, 'pit', seed, cx, cz, i).id;
                break;
            default: {
                // PATH：低频噪声成片替换为装饰平面地块（冰原/灰烬地/泥沼…）
                const bx = i % exports.BLOCKS_PER_SIDE;
                const bz = Math.floor(i / exports.BLOCKS_PER_SIDE);
                const wx = (cx * exports.BLOCKS_PER_SIDE + bx) * exports.BLOCK_SIZE;
                const wz = (cz * exports.BLOCKS_PER_SIDE + bz) * exports.BLOCK_SIZE;
                if (!portNear.has(i) && (0, TerrainNoise_1.vnoise)(wx * PATCH_FREQ, wz * PATCH_FREQ, seed + 7349) > PATCH_TAU) {
                    const decor = (0, TileGroups_1.drawGroundDecorTile)(panel, seed, cx, cz, i);
                    blockIds[i] = decor ? decor.id : Tiles_1.TILE_FLAT.id;
                }
                else {
                    blockIds[i] = Tiles_1.TILE_FLAT.id;
                }
                break;
            }
        }
...
// ============ L4 高度分配 ============
function assignHeights(blockIds, roles, ports, seed, chunkX, chunkZ) {
    ...
    for (let i = 0; i < 225; i++) {
        const def = (0, Tiles_2.tileById)(blockIds[i]);
        const p = def.physics;
        ...
        if (roles[i] === exports.ROLE_WALL && !allPorts.has(i)) {
            const bx = i % exports.BLOCKS_PER_SIDE;
            const bz = Math.floor(i / exports.BLOCKS_PER_SIDE);
...
function toChunkData(blockIds, tileHeights, chunkX, chunkZ) {
    ...
    for (let bz = 0; bz < exports.BLOCKS_PER_SIDE; bz++) {
        for (let bx = 0; bx < exports.BLOCKS_PER_SIDE; bx++) {
            ...
            const defT = (0, Tiles_2.tileById)(blockIds[ti]);
