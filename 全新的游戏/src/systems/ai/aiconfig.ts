...
/** ★ 远程·轻档：弩手。**射程 50m**（2026-09-20 用户定调：开火距离拉到 50m+）、射速最快、单发最低
 *  ★ 2026-09-18：改真弹道（`ranged`）—— 此前是"大 attackRange 的瞬时空打"，
 *    攻击没有任何可见效果；现在射程序化箭矢（飞行 → 接触命中）。 */
export const CROSSBOW_AI: AIConfig = mobAI({
  wanderSpeed: 2.2, chaseSpeed: 2.8, aggroRadius: 58,
  attackRadius: 50, attackRange: 50, loseRadius: 75,
  meleeDuration: 0.5, meleeDamage: 8,
  ranged: { speed: 34, lifetime: 2.4, damage: 8, spread: 0.045 },
});

/** ★ 远程·中档：扩音术士。射程 52m、伤害与节奏居中
 *  ★ 2026-09-18：改真弹道（法球 `skin:'fireball'`）—— 术士放法球，不再是瞬时空打 */
export const AMP_CASTER_AI: AIConfig = mobAI({
  wanderSpeed: 2, chaseSpeed: 2.4, aggroRadius: 60,
  attackRadius: 52, attackRange: 52, loseRadius: 80,
  meleeDuration: 0.8, meleeDamage: 10,
  ranged: { speed: 26, lifetime: 2.6, damage: 10, spread: 0.035, skin: 'fireball' },
});

/** ★ 远程·重档：战争术士。射程最远 55m + 单发最高，代价是慢与脆
 *  ★ 2026-09-18：改真弹道（大火球：更慢、更大、更痛） */
export const WAR_CASTER_AI: AIConfig = mobAI({
  wanderSpeed: 1.6, chaseSpeed: 2.0, aggroRadius: 64,
  attackRadius: 55, attackRange: 55, loseRadius: 85,
  meleeDuration: 1.2, meleeDamage: 20,
  ranged: { speed: 22, lifetime: 3.0, damage: 20, spread: 0.03, skin: 'fireball' },
});
