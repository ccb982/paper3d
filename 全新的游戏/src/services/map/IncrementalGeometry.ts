...
/**
 * ★ 物理 4m 分块切分（物理分块重构）：把整 chunk 顶面+侧壁字节切成 225 个
 * 4m 块 trimesh（slot = bz*15+bx；块 = 自有 16 个 1m cell + 4 条壁边；索引重定基
 * 到块局部）。布局按**输出** fine 掩码推导（topLayout/wallLayout 同一公式）——
 * 受影响 cell 恒 fine、基座可能 coarse，增量输出偏移会整体后移，绝不能拿基座
 * 布局切（受影响 cell/边字节由 buildXxxIncremental 保证与布局公式一致）。
 * blocks 缺省 = 全部；主线程只对受影响 slot 换 collider（O(块)）。
 */
export function partitionGroundTiles(
  ...
  const list = blocks ?? Array.from({ length: BPS * BPS }, (_, i) => i);
  const out: { slot: number; vertices: Float32Array; indices: Uint32Array }[] = [];
  for (const slot of list) {
    const lbx = slot % BPS, lbz = Math.floor(slot / BPS);
    // ---- 总量：16 个顶面 cell + 4 条自有壁边 ----
    let vCount = 0, iCount = 0;
    for (let lz = lbz * 4; lz < lbz * 4 + 4; lz++) {
      for (let lx = lbx * 4; lx < lbx * 4 + 4; lx++) {
        const c = lz * N + lx;
        vCount += tl.v[c];
    ...
    // ---- 壁边段（本块自有 4 条边；owner = (lbx,lbz)，与 wallLayout 遍历序一致） ----
    for (let dir = 0; dir < 4; dir++) {
      const s = (lbz * BPS + lbx) * 4 + dir;
