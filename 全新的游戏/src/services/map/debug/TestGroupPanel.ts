...
/** 兜底 seed（未传当天地图种子时；与旧 RasterMap 默认一致） */
const DEFAULT_SEED = 12345;

let el: HTMLDivElement | null = null;

/** 显示/刷新面板；groupKey 缺省时取实际生成 chunk 的生效组；
 *  seed = 当天地图种子（主种子 × 天数；缺省用旧默认 12345） */
export function showTestGroupPanel(groupKey?: string, seed?: number): void {
  const s = seed ?? DEFAULT_SEED;
  const key = groupKey ?? generateChunk(s, 0, 0).groupKey;
  const group = groupByKey(key);
  if (!group) return;

  // 实际 chunk 的块数直方图（与玩家所见同 seed 同坐标 → 数字即所见）
  const chunk = generateChunk(s, 0, 0);
