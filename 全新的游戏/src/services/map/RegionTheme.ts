...
/** 区域主题定义（加主题 = 加一行；id 即注册表下标） */
interface ThemeDef extends RegionParams {
  id: number;
  name: string;
}

/** 区域参数组（全部数值化——这是能双线性插值的前提） */
...
function nodeParams(seed: number, nx: number, ny: number): ThemeDef {
  const t = THEMES[Math.floor(hash2(nx, ny, seed + 777) * THEMES.length) % THEMES.length];
  return t;
}
 ...
 */
export function regionParamsAt(seed: number, x: number, z: number): RegionParams {
  ...
  const mix = (
    (a.hueShift * (1 - fx) + b.hueShift * fx) * (1 - fz) +
    (c.hueShift * (1 - fx) + d.hueShift * fx) * fz
  );
  const lerpN = (p: number, q: number, r: number, s: number) =>
    (p * (1 - fx) + q * fx) * (1 - fz) + (r * (1 - fx) + s * fx) * fz;
  return {
    hueShift: mix,
...
export function regionNameAt(seed: number, x: number, z: number): string {
