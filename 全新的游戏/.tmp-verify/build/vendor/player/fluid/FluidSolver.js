...
exports.FluidSolver = exports.defaultFluidConfig = void 0;
const THREE = __importStar(require("three"));
const FluidGrid_1 = require("./core/FluidGrid");
const GPUOps_1 = require("./core/GPUOps");
const AdvectionSolver_1 = require("./solvers/AdvectionSolver");
const LevelSetSolver_1 = require("./solvers/LevelSetSolver");
const FluidInjector_1 = require("./core/FluidInjector");
exports.defaultFluidConfig = {
...
class FluidSolver {
    constructor(renderer, config, resolution) {
        ...
        this._residualWidth = 0;
        this._residualHeight = 0;
        ...
        this._zeroObstacleFallback = null;
        // ==================== 调试：每帧场回读 ====================
        this._dbgFailD = false;
        this._dbgFailV = false;
        ...
        this.gpu = new GPUOps_1.GPUOps();
        this.advectionSolver = new AdvectionSolver_1.AdvectionSolver(renderer);
        this.levelSetSolver = new LevelSetSolver_1.LevelSetSolver(renderer, this.gpu);
        this.injector = new FluidInjector_1.FluidInjector(renderer, this.gpu);
        // 合成场景（全屏四边形）
    ...
    get phiGrid() {
        if (!this._phiGrid) {
            ...
            this._phiGrid = new FluidGrid_1.FluidGrid({ w, h }, 1, 'half-float');
            this.initPhiField();
        }
    ...
    rebuildGrids() {
        ...
        this.colorGrid = new FluidGrid_1.FluidGrid({ w, h }, 4, 'uint8');
        this.velocityGrid = new FluidGrid_1.FluidGrid({ w, h }, 2, 'half-float');
        this.pressureGrid = new FluidGrid_1.FluidGrid({ w, h }, 1, 'half-float');
        this.densityGrid = new FluidGrid_1.FluidGrid({ w, h }, 1, 'uint8');
        // ★ 散度源场（爆炸/源汇压力源项；solvePressure 消费后清空）
        this.divergenceGrid = new FluidGrid_1.FluidGrid({ w, h }, 1, 'half-float');
        if (this._phiGrid) {
     ...
     */
    loadResidual(data, width, height) {
    ...
    uploadResidualToColorGrid() {
        if (!this._pendingResidual)
            return;
        const w = this._residualWidth;
        const h = this._residualHeight;
        const tex = new THREE.DataTexture(this._pendingResidual, w, h, THREE.RGBAFormat, THREE.UnsignedByteType);
     ...
     */
    setBaseHsl(data, width, height) {
        if (width <= 0 || height <= 0 || data.length < width * height * 4) {
            this.clearBaseHsl();
            return;
        }
        this.baseHslTex?.dispose();
        this.baseHslTex = new THREE.DataTexture(data, width, height, THREE.RGBAFormat, THREE.FloatType);
    ...
    setObstacleTexture(tex) {
        this.obstacleTex = tex;
    }
    getObstacleTex() {
        return this.obstacleTex || this.getZeroObstacleTexFallback();
    }
    getZeroObstacleTexFallback() {
        if (!this._zeroObstacleFallback) {
            this._zeroObstacleFallback = new THREE.DataTexture(new Uint8Array([0]), 1, 1, THREE.RedFormat, THREE.UnsignedByteType);
    ...
    applyPhiCorrection() {
        ...
        this.levelSetSolver.applyPhiCorrection(this._phiGrid, this.getObstacleTex(), clampAir, maxAirPhi, compensate, compRate);
    }
    // ==================== 场清零 ====================
    /**
    ...
    applyGravity(dt) {
        ...
        if (g.x === 0 && g.y === 0)
            return;
        this.injector.injectVelocity(this.velocityGrid, { x: g.x * dt, y: g.y * dt }, { global: true, obstacle: this.obstacleTex || undefined });
    }
    // ==================== 持续注入源 ====================
    processContinuousSources(dt) {
        ...
        for (let i = 0; i < this.config.continuousSources.length; i++) {
            ...
            if (int && int.onDuration > 0) {
                ...
                if (period <= 0)
                    continue;
                if (this.time % period >= int.onDuration)
                    continue; // 间歇期：本帧不注入
            }
            const config = { ...src };
            // 路径点插值
            ...
            if (!isFinite(config.position.x) || !isFinite(config.position.y))
                continue;
            if (!isFinite(config.velocity.x) || !isFinite(config.velocity.y))
                continue;
            const opts = {
            ...
            if (!isScalar && config.color) {
                this.injector.injectColor(this.colorGrid, { h: config.color[0], s: config.color[1], l: config.color[2], a: config.color[3] }, config.rate ?? 1.0, opts, ch);
            }
            // 速度注入
    ...
    processInjectionQueue() {
        ...
        for (const inj of this.injectionQueue) {
            ...
            if (!isScalar && inj.color) {
                this.injector.injectColor(this.colorGrid, { h: inj.color[0], s: inj.color[1], l: inj.color[2], a: inj.color[3] }, inj.rate ?? 1.0, opts, ch);
            }
            else if (isScalar) {
        ...
        for (let i = this.activeExplosions.length - 1; i >= 0; i--) {
            ...
            if (ex.createWater && waterMult > 0) {
                ...
                if (isScalar) {
                    ...
                    this.injector.injectColor(this.colorGrid, { h: wc[0], s: wc[1], l: wc[2], a: wc[3] }, rate, opts, ch);
                }
            }
        }
    }
    ...
    // ==================== 压力投影（红-黑 SOR）====================
    solvePressure(iterations, omega, phiTex = null) {
        const { w, h } = this.config.resolution;
        if (w === 0 || h === 0)
            return;
    ...
    runSORPass(color, omega, phiTex = null) {
        const { w, h } = this.config.resolution;
    ...
    step(dt) {
        ...
        if (cfg.enableAdvection) {
            // 速度自平流
            this.advect(this.velocityGrid, this.velocityGrid.read, dt, { r: true, g: true, b: false, a: false }, 'clamp', false);
            // 颜色/density 平流
            if (isScalar) {
                this.advect(this.densityGrid, this.velocityGrid.read, dt, { r: true, g: false, b: false, a: false }, 'clamp', false);
                this.decayDensity();
            }
            else {
                this.advect(this.colorGrid, this.velocityGrid.read, dt, cfg.channels, cfg.colorBoundaryMode || 'clamp', true);
            }
        }
        ...
        if (ls?.enabled && this._phiGrid) {
            ...
            if (constrain) {
                ...
                this.levelSetSolver.applyLiquidConstraint(targetGrid, this._phiGrid.read, this.getObstacleTex(), mode, band, 0.02);
            }
            else {
                this.levelSetSolver.advectPhi(this._phiGrid, this.velocityGrid.read, dt, this.getObstacleTex());
            ...
            if (ls.surfaceTension > 0) {
                this.levelSetSolver.applySurfaceTension(this.velocityGrid, this._phiGrid.read, this.getObstacleTex(), ls.surfaceTension, dt, band);
            }
        }
        ...
        if (maxVel > 0 && isFinite(maxVel))
            this.clampVelocity(maxVel);
        // 6.5 ★ 外向速度抑制（确定性收拢；与张力符号/φ噪声无关）
        if (ls?.enabled && this._phiGrid) {
            const od = ls.outwardDamping ?? 0;
            if (od > 0) {
                const band = ls.narrowBandWidth ?? ls.smoothingRadius ?? 5;
                this.levelSetSolver.applyOutwardVelDamping(this.velocityGrid, this._phiGrid.read, this.getObstacleTex(), band, od);
            }
        }
        this.time += dt;
    }
    ...
    composite() {
        ...
        if (hasBase) {
            ...
            if (this.config.advectionMode === 'scalar') {
                ...
                u.uChannelMul.value.set(sc.hMultiplier, sc.sMultiplier, sc.lMultiplier, sc.aMultiplier);
    ...
    debugReadFields(tag = '') {
        const { w, h } = this.config.resolution;
        const px = (col, row) => `(${((col + 0.5) / w).toFixed(2)},${(1 - (row + 0.5) / h).toFixed(2)})`;
