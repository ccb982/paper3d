...
class FluidInjector {
    ...
    getDummyWhiteTex() {
        if (!this.dummyWhiteTex) {
            this.dummyWhiteTex = new THREE.DataTexture(new Uint8Array([255, 255, 255, 255]), 1, 1, THREE.RGBAFormat, THREE.UnsignedByteType);
     ...
     */
    injectDivergence(grid, divergence, options = {}) {
     ...
     * 累积模式：叠加到源场现有值。
     */
    injectDivergenceSource(grid, divergence, options = {}) {
        const { position = { x: 0.5, y: 0.5 }, radius = 0.1, mask, global = false, obstacle } = options;
     ...
     *              调用方负责完成 dt / 包络缩放。
     */
    injectRadialVelocity(grid, speed, options = {}) {
     ...
     */
    injectColor(grid, color, rate, options = {}, channelMask = { r: true, g: true, b: true, a: true }) {
        const { position = { x: 0.5, y: 0.5 }, radius = 0.1, mask, global = false, obstacle } = options;
     ...
     */
    injectColorTexture(grid, colorTex, maskTex, rate) {
        const clampedRate = Math.min(1.0, Math.max(0.0, rate));
     ...
     */
    injectDensity(grid, value, rate, options = {}) {
        const { position = { x: 0.5, y: 0.5 }, radius = 0.1, mask, global = false, obstacle } = options;
     ...
     */
    injectVelocity(grid, velocity, options = {}) {
        const { position = { x: 0.5, y: 0.5 }, radius = 0.1, mask, global = false, obstacle } = options;
     ...
     */
    clampVelocity(grid, maxSpeed = 5000) {
...
exports.FluidInjector = FluidInjector;
