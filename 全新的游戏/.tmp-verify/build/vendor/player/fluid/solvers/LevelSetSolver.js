...
class LevelSetSolver {
     ...
     */
    advectPhi(phiGrid, velocityTex, dt, obstacleTex) {
     ...
     *       新注入的液体才会长出表面。
     */
    reinit(phiGrid, obstacleTex, iterations, dtPseudo = 0.5) {
        // ① φ₀ 快照（复制当前 φ → phi0Tex）
    ...
    runReinitPass(phiGrid, obstacleTex, isRed, dtPseudo) {
        const w = phiGrid.resolution.w;
        const h = phiGrid.resolution.h;
        const mat = this.gpu.getMaterial(`levelset_reinit_godunov_${isRed ? 'r' : 'b'}`, {
            uPhi: { value: phiGrid.read },
     ...
     */
    applySurfaceTension(velocityGrid, phiTex, obstacleTex, sigma, dt, smoothingRadius) {
     ...
     */
    applyOutwardVelDamping(velocityGrid, phiTex, obstacleTex, band, strength) {
        if (strength <= 0)
            return;
        const w = velocityGrid.resolution.w;
        const h = velocityGrid.resolution.h;
     ...
     */
    applyLiquidConstraint(targetGrid, phiTex, obstacleTex, mode, band, fadeTo) {
     ...
     */
    applyPhiCorrection(phiGrid, obstacleTex, clampAir, maxAirPhi, compensateWater, compRate) {
     ...
     */
    initPhiField(phiGrid, sourceTex, mode, scale) {
     ...
     */
    applyDespeckle(phiGrid, obstacleTex) {
        const w = phiGrid.resolution.w;
        const h = phiGrid.resolution.h;
...
exports.LevelSetSolver = LevelSetSolver;
