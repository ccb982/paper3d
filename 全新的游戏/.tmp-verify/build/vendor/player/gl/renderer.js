...
exports.buildEntityMesh = buildEntityMesh;
exports.renderFrameData = renderFrameData;
exports.createThreeContext = createThreeContext;
const THREE = __importStar(require("three"));
const earcut_1 = __importDefault(require("earcut"));
const GameRenderer_1 = require("../../../services/render/GameRenderer");
function triangulateBoundary(boundary) {
    const flatVertices = [];
    const holeIndices = [];
    ...
    for (let ringIdx = 0; ringIdx < boundary.length; ringIdx++) {
        ...
        if (ringIdx > 0)
            holeIndices.push(totalVertexCount);
        for (const v of ring) {
    ...
    let indices;
    try {
        const result = (0, earcut_1.default)(flatVertices, holeIndices.length > 0 ? holeIndices : undefined, 2);
        indices = Array.from(result).filter((i) => i >= 0 && i < totalVertexCount);
        if (indices.length < 3)
            throw new Error('earcut returned too few indices');
    }
    catch {
        if (boundary[0].length < 3)
            return null;
        indices = [];
        for (let i = 1; i < boundary[0].length - 1; i++)
...
function makeFillMaterial(displacementTexture, vertexCount, totalFrames) {
...
function makeColorMaterial(displacementTexture, vertexCount, totalFrames) {
    return new THREE.ShaderMaterial({
        vertexShader: /* glsl */ `
      uniform sampler2D uDisplacementTex;
      uniform float uTime;
...
function buildEntityMesh(entity, ftxBbox, displacementTexture, _vertexCount, totalFrames, canvasSize, canvasSizeY) {
    const boundary = entity.boundary.map((ring) => ring.map((p) => [p.x, p.y]));
...
function renderFrameData(data, animationTime, vatFps, transform, renderer, scene, camera, fluidTexture) {
    ...
    for (const em of data.entities) {
        ...
        if (useFluid && fluidTexture)
            cm.uniforms.uFluidTex.value = fluidTexture;
        // bbox 映射（世界坐标 → 纹理 bbox 空间）
        // bbox 映射（世界坐标 → 纹理 bbox 空间）
        // ★ X/Y 分别用帧宽/帧高归一化（此前 Y 也除 frameWidth → 非正方形帧
        //   （如 424×512）时 bbox 被压扁 + 底部内容被裁掉）
        cm.uniforms.uBboxOffset.value.set(em.texBbox.x / em.frameWidth, em.texBbox.y / em.frameHeight);
        cm.uniforms.uBboxScale.value.set(em.texBbox.w / em.frameWidth, em.texBbox.h / em.frameHeight);
        // 用户底图变换
        cm.uniforms.uTexOffset.value.set(data.textureOffset.x, data.textureOffset.y);
        cm.uniforms.uTexScale.value.set(data.textureScale.x, data.textureScale.y);
...
function createThreeContext(sizeOrCanvas) {
    ...
    (0, GameRenderer_1.applyShaderDebug)(renderer);
