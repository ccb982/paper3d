...
class FtxAsset {
    constructor(buffer) {
        ...
        this.hitDyeSpreadSpeed = 40;
        const decoded = (0, ftx_1.decodeMultiFrame)(buffer);
        this.frames = decoded.frames;
        this.palette = decoded.palette;
        this.frameCount = decoded.frames.length;
        this.resolver = new frameResolver_1.FrameResolver(this.frames.map((f) => f.name));
        for (let i = 0; i < this.frames.length; i++) {
            const pair = this.buildFramePair(i);
            if (pair)
                this._frameTextures.push(pair);
        }
    }
    ...
    buildFramePair(index) {
        const frame = this.frames[index];
        if (!frame)
            return null;
        const baseHsl = (0, ftx_1.buildBaseHslData)(frame, this.palette);
        const residualData = (0, ftx_1.buildResidualData)(frame);
        if (!baseHsl || !residualData)
            return null;
        const base = new THREE.DataTexture(baseHsl.data, baseHsl.width, baseHsl.height, THREE.RGBAFormat, THREE.FloatType);
        base.flipY = false;
        ...
        const residual = new THREE.DataTexture(residualData.data, residualData.width, residualData.height, THREE.RGBAFormat, THREE.UnsignedByteType);
     ...
     */
    static createCompositeMaterial() {
     ...
     */
    getFluidEffect(index, renderer, physics) {
        const frame = this.frames[index];
        if (!frame)
            return null;
        ...
        if (cached)
            return cached;
        const effect = new FluidEffect_1.FluidEffect(renderer, physics, frame, this.palette, []);
    ...
    createDeathFluidEffect(renderer, frameIndex) {
        ...
        return new FluidEffect_1.FluidEffect(renderer, physics, frame, this.palette, []);
    }
    /**
...
async function maybeGunzip(buffer) {
    ...
    try {
        ...
        throw new Error('Gzip 解压失败: ' + err.message);
    }
}
