...
exports.HitEffectView = void 0;
const THREE = __importStar(require("three"));
const variantGenerator_1 = require("./variantGenerator");
...
/** 外置残差层 base64 → 画布 */
function buildResidualImage(res) {
    const c = document.createElement('canvas');
    c.width = res.width;
    c.height = res.height;
    c.getContext('2d').putImageData(new ImageData(new Uint8ClampedArray(base64ToBytes(res.dataBase64)), res.width, res.height), 0, 0);
    return c;
}
class HitEffectView {
    constructor(scene, shapes, options) {
        this.defs = [];
        this.ftxImages = new Map();
        this.residualImages = new Map();
        this.variants = [];
        this.totalDuration = 0;
        this.elapsed = -1; // <0 = 未开始
        /** ★ 统一烘焙缩放：按「最大轮廓跨度 × 最大外扩 × 旋转余量」预计算，
         *   保证任何变体/任意旋转角都完整落在画布内（不再裁剪溢出） */
        this.fitScale = 1;
        /** ★ 跟随偏移（击中点相对被跟随实体的偏移；实体槽每帧传实体位置时叠加） */
        this.followOffset = { x: 0, y: 0, z: 0 };
        ...
        for (const sh of shapes) {
            if (sh.fillMode === 'ftx' && sh.ftx)
                this.ftxImages.set(sh.name, buildFtxImage(sh.ftx));
            if (sh.residualLayer)
                this.residualImages.set(sh.name, buildResidualImage(sh.residualLayer));
        }
    ...
    play(x, y, z) {
        const seed = (0, variantGenerator_1.randomSeed)();
        this.variants = this.defs.map((d, i) => (0, variantGenerator_1.generateVariant)(d, (0, variantGenerator_1.shapeSeed)(seed, i)));
        this.totalDuration = Math.max(...this.defs.map(variantGenerator_1.variantDuration));
        this.elapsed = 0;
    ...
    drawFrame(t) {
        ...
        for (let i = 0; i < this.defs.length; i++) {
            ...
            const pose = (0, variantGenerator_1.tickVariant)(v, t, sh);
            let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
...
exports.HitEffectView = HitEffectView;
