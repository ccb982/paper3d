...
exports.decodeMultiFrame = decodeMultiFrame;
exports.buildBaseHslData = buildBaseHslData;
exports.buildResidualData = buildResidualData;
exports.buildFrameTexture = buildFrameTexture;
...
function decodeMultiFrame(buffer) {
    ...
    offset += 4;
    if (magic !== 0x46545833)
        throw new Error('无效的 FTX3 格式 (Magic 不匹配)');
    const version = view.getUint8(offset);
    offset += 1;
    if (version !== 3)
        throw new Error(`不支持的 FTX 版本: ${version}`);
...
function buildResidualData(frame) {
    ...
    //   与主绘画页面/编辑器约定统一：合成 alpha 用「残差非中性度」判定，
    //   shape 外中性 → 不显示 → 纯透明背景；shape 内真实残差覆盖（alpha=255）。
    for (let i = 0; i < totalPixels; i++) {
        data[i * 4] = 128;
        data[i * 4 + 1] = 128;
        data[i * 4 + 2] = 128;
    ...
    for (let i = 0; i < totalPixels; i++) {
        ...
        data[idx4 + 1] = g8;
        data[idx4 + 2] = b8;
        // ★ alpha = 区域存在度（与编辑器/主绘画页面统一）：shape 内 255。
        //   合成 alpha 用「残差非中性度」(resAct) 判定显示，不用本通道；
        //   本通道仅作约定（此前 128 → 合成 max(baseA,residual.a) 时 shape 外
        //   baseA=0 → 0.5 → 半透明黑）
        data[idx4 + 3] = 255;
    }
    return { data, width: w, height: h };
...
function buildFrameTexture(frame, palette) {
    ...
    if (!baseHsl || !residual)
        throw new Error('无法构建 HSL/残差数据');
    const baseTex = new THREE.DataTexture(baseHsl.data, baseHsl.width, baseHsl.height, THREE.RGBAFormat, THREE.FloatType);
    baseTex.flipY = false;
    ...
    const resTex = new THREE.DataTexture(residual.data, residual.width, residual.height, THREE.RGBAFormat, THREE.UnsignedByteType);
    resTex.flipY = false;
