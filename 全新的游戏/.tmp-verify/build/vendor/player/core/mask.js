...
exports.applyDistortionCPU = applyDistortionCPU;
function hashCPU(p) {
    const x = Math.sin(p) * 43758.5453;
    return x - Math.floor(x);
}
function smoothNoiseCPU(x, y, seed) {
...
function applyWaveCPU(points, op, time) {
...
function applyTurbulentCPU(points, op, time) {
    const amp = op.amplitude || 0.05;
...
function applyTwirlCPU(points, op, time) {
...
function applyDistortionCPU(points, maskEffect, time) {
    ...
    for (const op of maskEffect.distortions) {
        ...
        switch (op.type) {
            case 'wave':
                pts = applyWaveCPU(pts, op, time);
                break;
            case 'turbulent':
                pts = applyTurbulentCPU(pts, op, time);
                break;
            case 'twirl':
                pts = applyTwirlCPU(pts, op, time);
                break;
        }
    }
    return pts;
