...
exports.buildDisplacementTextureData = buildDisplacementTextureData;
const mask_1 = require("./mask");
function buildDisplacementTextureData(boundary, maskEffect, _canvasWidth, _canvasHeight, fixedVertices, loopFrames = 30) {
    ...
    for (let frame = 0; frame < totalFrames; frame++) {
        ...
        if (maskEffect) {
            for (const ring of boundary) {
                const distorted = (0, mask_1.applyDistortionCPU)(ring, maskEffect, t);
                distortedAll.push(...distorted);
            }
        }
        else {
            for (const v of allVertices)
                distortedAll.push({ x: v.x, y: v.y });
        }
