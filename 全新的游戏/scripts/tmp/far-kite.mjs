...
// 把玩家挪到远处（脱离落点扫描范围），并强制总攻（近战会追，远程不该贴脸）
const t0 = await page.evaluate(() => {
  const m = window.__ppMode(), s = window.__swarm;
  m.player.position.x += 220;   // 远离落点：posts 全部超射程
  s.commander.setPosture('assault');
  const q = [...s.squads.all()].find((x) => x.type === 'ranged');
  return { id: q?.id ?? -1, arrows0: window.__arrows };
});
const sample = (label) => page.evaluate((l, sid) => {
  const m = window.__ppMode(), s = window.__swarm;
  const q = s.squads.get(sid);
  if (!q) return { l, none: true };
  let cx = 0, cz = 0, n = 0;
...
const rows = [];
for (let i = 0; i < 6; i++) { await new Promise((r) => setTimeout(r, 4000)); rows.push(await sample(`t=${(i + 1) * 4}s`)); }
console.log(JSON.stringify({ t0, rows, errs: errs.length }, null, 2));
await browser.close();
