...
export const applyGravityToCharacter = (
  ...
  let groundY = GROUND_HEIGHT;
  
  if (getHeightAt) {
    const terrainHeight = getHeightAt(newPosition.x, newPosition.z);
    const feetHeight = terrainHeight + 1.5 - 0.75;
    const targetCharacterY = feetHeight + CHARACTER_HEIGHT / 2;
    
    // 确保角色不会低于地形表面
    if (newPosition.y < targetCharacterY) {
      newPosition.y = targetCharacterY;
    }
    
    groundY = targetCharacterY;
  }
