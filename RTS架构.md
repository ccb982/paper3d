...
- **全环仍分 8 个扇区**（就是现有 `FORTIFY_SECTORS = 8` 口径）；**从中选 1~3 个作为主攻方向**（按难度）——
  ...
  **（后续待实现）未入选扇区可放少量小队“偷袭”**（绕袭/骚扰；少量、不占主力编制）——本期只记档，不实现。
...
- **同一高原/山顶（与角色所在层连通的高台）不得纳入防区**——上去要绕路；防区偏防守，不强占角色所在的高原/山顶。
- 防区 = **山脚下的包围圈**（同层可达、无需爬坡/绕路）；部署点必须与角色层连通（`canReach` 同层口径）。
- 实现口径：以角色（舰船）所在层为基准，用可行性表/层判定剔除“需跨坡才能到”的同层高台区域（`layerAt`/高差＞阈值 或 上坡才能进入 → 排除）。
